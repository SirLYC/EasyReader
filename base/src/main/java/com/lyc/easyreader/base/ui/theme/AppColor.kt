package com.lyc.easyreader.base.ui.theme

import com.lyc.easyreader.base.R
import com.lyc.easyreader.base.ui.getColorRes

/**
 * Created by Liu Yuchuan on 2020/1/18.
 */
val color_bg by lazy { getColorRes(R.color.colorBg) }
val color_yellow by lazy { getColorRes(R.color.colorYellow) }
val color_orange by lazy { getColorRes(R.color.colorOrange) }
val color_orange_deep by lazy { getColorRes(R.color.colorDeepOrange) }
val color_gray by lazy { getColorRes(R.color.colorGray) }
val color_primary_text by lazy { getColorRes(R.color.colorPrimaryText) }
val color_secondary_text by lazy { getColorRes(R.color.colorSecondaryText) }
val color_divider by lazy { getColorRes(R.color.colorDivider) }
val color_light_blue by lazy { getColorRes(R.color.colorLightBlue) }
